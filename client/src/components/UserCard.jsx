...
const UserCard = ({user}) => {
    ...
    const handleFollow = async()=>{
        ...
        console.log('follow user:', user.full_name)
    }

    const handleConnectionRequest = async()=>{
        // API Call to send connection request
        console.log('send connection request to:', user.full_name)
    }
  ...
  return (
    <div key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md'>
        ...
        <div className='flex mt-4 gap-2'>
            ...
            <button onClick={handleFollow} disabled={currentUser?.following.includes(user._id)} className={`w-full py-2 rounded-md  flex justify-center items-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition text-white  ${currentUser?.following.includes(user._id) ? "cursor-not-allowed": "cursor-pointer"}`}>
