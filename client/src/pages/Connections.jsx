...
const Connections = () => {
  ...
  const loadedCount = dataArray.map((item, index)=>(
  ...
  const tabs = dataArray.map((tab)=>(
    ...
    const loadedConnections = dataArray.find((item)=>item.label === currentTab).value.map((user)=>(
